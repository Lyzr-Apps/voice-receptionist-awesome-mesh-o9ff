...
export default function Page() {
  ...
  // Derive displayed data based on sample toggle
  const displayedOrders = showSampleData ? [...SAMPLE_ORDERS, ...orders] : orders
  const displayedCalls = showSampleData ? [...SAMPLE_CALLS, ...calls] : calls
  ...
  const handleToggleStar = useCallback((callId: string) => {
    setCalls(prev => prev.map(c => c.id === callId ? { ...c, starred: !c.starred } : c))
  }, [])

  const handleCallEnd = useCallback((transcript: TranscriptEntry[]) => {
    if (!Array.isArray(transcript) || transcript.length === 0) return

    const orderId = `ORD-${String(orders.length + SAMPLE_ORDERS.length + 1).padStart(3, '0')}`
    const callId = `CALL-${String(calls.length + SAMPLE_CALLS.length + 1).padStart(3, '0')}`
    ...
    <ErrorBoundary>
      <div className="min-h-screen bg-background text-foreground flex">
        ...
        <main className="flex-1 overflow-y-auto min-h-screen">
          {/* Top bar */}
          <header className="sticky top-0 z-10 px-6 py-3 flex items-center justify-between border-b border-border/30" style={{ background: 'hsl(350 35% 97% / 0.8)', backdropFilter: 'blur(12px)', WebkitBackdropFilter: 'blur(12px)' }}>
